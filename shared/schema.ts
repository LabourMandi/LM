...
export const users = pgTable("users", {
  ...
  role: varchar("role", { enum: ["worker", "employer", "admin", "contractor", "client"] }).default("employer"),
  bio: text("bio"),
...
// Machinery/Heavy Equipment for rental and sale
export const machinery = pgTable("machinery", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ownerId: varchar("owner_id").notNull().references(() => users.id),
  name: varchar("name").notNull(),
  type: varchar("type").notNull(),
  rentalType: varchar("rental_type", { enum: ["rent", "sell", "both"] }).default("rent"),
  pricePerDay: decimal("price_per_day", { precision: 10, scale: 2 }),
  sellPrice: decimal("sell_price", { precision: 12, scale: 2 }),
  condition: varchar("condition", { enum: ["excellent", "good", "fair", "needs_repair"] }).default("good"),
  modelYear: integer("model_year"),
  images: text("images").array(),
  location: varchar("location"),
  description: text("description"),
  availability: varchar("availability", { enum: ["available", "rented", "sold", "maintenance"] }).default("available"),
  rating: decimal("rating", { precision: 3, scale: 2 }).default("0"),
  totalRentals: integer("total_rentals").default(0),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),

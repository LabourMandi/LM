...
// PATCH: smart-profile - import smart profile routes
import { setupSmartProfileRoutes } from "./ext/smart-profile";
// END PATCH
...
export async function registerRoutes(
  ...
  app.post("/api/ai/match-workers", isAuthenticated, async (req: any, res) => {
    try {
      if (!openai) {
        return res.status(503).json({ message: "AI features are not available" });
      }
  ...
  // PATCH: smart-profile - register isolated smart profile routes
  setupSmartProfileRoutes(app, storage, isAuthenticated);
  // END PATCH
